...
@app.post("/negotiate")
def negotiate(sla: dict, fairness: dict):
    """
    Generate negotiation suggestions based on SLA and fairness score.
    Expects JSON body with 'sla' and 'fairness' fields.
    """
    try:
        points = generate_negotiation_points(sla, fairness)
        return {"negotiation_points": points}
    except Exception:
        traceback.print_exc()
        return {"error": "Failed to generate negotiation points"}
